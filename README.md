...
| `/platform/*` | Web（桌面） | 平台管理员 | ✅ 已上线 |
| `miniapp-repair/` | 小程序 | 客户 + 师傅 | 🆕 V0.5.0 |

### 未来规划

**生产环境建议客户和师傅使用独立微信小程序**，Web 继续承载管理员和平台方：

- 🆕 独立售后维修小程序（一个 AppID 服务多个租户）
- 客户和师傅共用同一小程序，通过扫码进入各自流程
- **不会**把维修系统塞进 eat-what（今天吃啥）小程序
- H5 客户端和 H5 师傅端保留作为备用入口和降级方案
- 现有后端 API 对小程序 MVP 适配度 **100%**，无需新增接口

> 详细方案见 [小程序化方案设计](docs/miniapp-plan.md) 和 [接口适配分析](docs/miniapp-api-gap.md)。
...
| V0.5.0 | 售后维修小程序 MVP（原生微信小程序，客户+师傅端） | 🆕 |
| V0.5.1+ | 图片上传 / 微信能力 / 小程运营 | 📋 |
